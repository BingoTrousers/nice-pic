...
function PostPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      <Link to="/" className="mb-2 inline-block py-2 pr-4 text-sm text-muted-foreground hover:text-foreground">
        ← back
      </Link>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {post.tags.map((tag: string) => (
          <Link
            ...
            to="/"
            search={{ tags: tag }}
            className="rounded border bg-card px-3 py-2 font-mono text-xs hover:bg-accent"
          >
            {tag}
          </Link>
        ))}
